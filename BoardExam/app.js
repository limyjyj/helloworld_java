...
app.get('/', function(req, res){
	connection.query('select * from board', function(err, results, fields){
		if(err) res.send(err);
		else console.log(JSON.stringify(results)),res.render('index', {title:'게시판 글목록', boards:results});
	});
});
...
app.get('/detail/:no', function(req,res){
	connection.query('select * from board where no=?', req.params.no,
			function(err, results, fields){
			if(results.length > 0){
				res.render('detail', {title:'게시판 상세정보', board:results[0]});
			}else{
				res.redirect('/');
			}
			});
